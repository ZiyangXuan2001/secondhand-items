...
- `negotiable` 为可选布尔值，只有用户明确指定可议价时才设置为 `true`。缺省或 `false` 不显示标签，不能根据低价或物品类别自行推断。
- 在售且 `negotiable: true` 时，在物品名称后显示绿色「可议价」标签；售出后不显示此标签，保留数据值。用户说不可议价时改为 `false`。

## 物品分类

- 每件物品填写 `category`：工作用品、生活电器、家具收纳、桌游玩具、乐器；无法判断时用其他。显示器归工作用品，家电归生活电器，桌椅及置物架归家具收纳，桌游和儿童科普望远镜归桌游玩具，电子琴归乐器。用户指定分类时以用户为准。
- 名称下显示分类标签，顶部提供分类筛选；保留一件物品一行。默认同类相邻，已售出仍放最后；分类可与搜索和出售状态筛选组合使用。分类按钮数量按当前搜索及出售状态统计。
- 分类颜色固定：工作用品蓝色、生活电器青色、家具收纳橙色、桌游玩具紫色、乐器粉色、其他灰色。分类筛选按钮、名称下标签及每行左侧色条保持一致；同时保留类别文字，不能只依赖颜色。亮绿色的可议价标签保持独立。
